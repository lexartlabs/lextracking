  ...
  Module.controller('ReportsCtrl', ['$scope','$interval', '$state', '$rootScope', '$filter', '$timeout', 'UserServices', 'ClientServices', 'ProjectsServices', 'TracksServices', 'ngDialog','WeeklyHourServices', 'TasksServices', function($scope,$interval, $state, $rootScope, $filter, $timeout, UserServices, ClientServices, ProjectsServices, TracksServices, ngDialog,WeeklyHourServices,TasksServices) {
          ...
          angular.forEach(weeklyHours, function(value, key) {
                  if (value.idUser == idHourCost) {
                    ...
                    $scope.finalTotal = cost.value;
                      ...
                      console.log("COSTO TAREA AUTOMATICA en error", cost.value.result2);
                    }
                  }
              });
                  ...
                  if (value.idUser == idHourCost) {
                    ...
                    if(cost.value != undefined){
                      $scope.finalTotal = cost.value;
                        ...
                        console.log("COSTO TAREA trello en error", cost.value.result3);
                      }
                  }
              });
              console.log(weeklyHours);
    ...
    if ($rootScope.isClient=="false") {
      ...
      ClientServices.find(0, '', function(err, clients) {
        if (!err) {
          clients.unshift({ "id": 0, "name": $filter("translate")("reports.all") });
    ...
    $scope.getProjects = function (idClient) {
      if (idClient) {
        ProjectsServices.getProjectsByClient(idClient, function (err, projects) {
          if (!err) {
            projects.unshift({ "id": 0, "name": $filter("translate")("reports.all") });
   ...
   ProjectsServices.find(0, '', function (err, projects) {
    ...
    $scope.$watch('filter.startTime', function(newValue, oldValue) {
      ...
      console.log("test",moment(),moment($scope.filter.startTime,"DD/MM/YYYY"),moment($scope.filter.startTime,"DD/MM/YYYY").diff(moment(),'month'));
        ...
        if (!err) {
          ...
          tracks.forEach(function(track){
            ...
            if(userRole == 'admin'){
              ...
              if($scope.tableTrack.length < 1){
                $scope.tableTrack.push({'idProyecto':track.idProyecto,'duration':track.duration,'subTotalCost':parseInt(track.trackCost ? track.trackCost : 0),'tracks':[track]})
              }else{
                    var exist = false;
                    $scope.tableTrack.forEach(function(element){
          ...
          $scope.tableTrack.forEach(function(el){
            ...
            el.tracks.forEach(function(track,index){ // recorremos los tracks
              if(el.byProject.length < 1){  
                el.byProject.push({'idProyecto':track.idProyecto,'projectName':track.projectName,'duration':track.duration,'subTotalCost':parseInt(track.trackCost ? track.trackCost : 0),'tracks':[track]})
              }else{
                var exist = false;
          ...
          for (var k in $scope.subtotals) {
            if ($scope.subtotals.hasOwnProperty(k)) {
              ...
              $scope.sumHours.push({'idUser':k,'duration':$scope.subtotals[k]});
            }
          }
          ...
          for (var i = 0; i < $scope.sumHours.length; i++){
            ...
            mss = {'idUser':$scope.sumHours[i].idUser,'mss':subcost._milliseconds};
            getSubTotalCost(mss);
          }
        ...
        if (!err) {
            ...
            if(userRole == 'admin'){
              ...
              if($scope.tableTrackAuto.length < 1){
                $scope.tableTrackAuto.push({'idUser':track.idUser,'idProyecto':track.idProyecto,'duration':track.durations,'subTotalCost':parseInt(track.trackCost ? track.trackCost : 0),'tracks':[track]})
                    ...
                    if(exist === false){
                      $scope.tableTrackAuto.push({'idProyecto':track.idProyecto,'duration':track.durations,'subTotalCost':parseInt(track.trackCost ? track.trackCost : 0),'tracks':[track]})
                    }
              }
            }
          ...
          _.each(tracks, function (track){
            ...
            if (track.durations.indexOf('-') !== -1) {
              track.durations = getTotalTimeAuto((now - track.startTime)/1000);
              msc += (now - track.startTime);
              $scope.subtotals2[track.projectError] += (now - track.startTime);
            } else {
              msc += (track.endTime - track.startTime);
              $scope.subtotals2[track.projectError] += (track.endTime - track.startTime);
            }
          ...
          for (var k in $scope.subtotals2) {
            if ($scope.subtotals2.hasOwnProperty(k)) {
              $scope.subtotals2[k] = getTotalTimeAuto($scope.subtotals2[k]/1000);
      ...
      if (!err) {
        ...
        var tempTotal = 0;
          tracks.forEach(function(track){
            ...
            if(userRole == 'admin'){
              ...
              if($scope.tableTrackTrello.length < 1){
                $scope.tableTrackTrello.push({'idProyecto':track.idUser,'idProyecto':track.idProyecto,'clientName': track.client,'duration':track.duration,'subTotalCost':parseInt(track.trackCost ? track.trackCost : 0),'tracks':[track]})
              }else{
                    var exist = false;
                    $scope.tableTrackTrello.forEach(function(element){
                      if(element.idProyecto == track.idProyecto && exist == false){
                    ...
                    if(exist === false){
                      $scope.tableTrackTrello.push({'idProyecto':track.idProyecto,'clientName': track.client,'duration':track.durations,'subTotalCost':parseInt(track.trackCost ? track.trackCost : 0),'tracks':[track]})
                    }
                  }
            }
        ...
        for (var k in $scope.subtotals3) {
          if ($scope.subtotals3.hasOwnProperty(k)) {
            $scope.subtotals3[k] = getTotalTimeTrello($scope.subtotals3[k]/1000);
    ...
    $scope.updateTrelloTrackDuration = function (trelloTrack) {
      if (typeof trelloTrack.startTime === 'string' && typeof trelloTrack.endTime === 'string') {
        ...
        endTime = new Date(endTime).getTime();
        trelloTrack.durations  = getTotalTimeAuto((endTime - startTime)/1000);
      }
    }
